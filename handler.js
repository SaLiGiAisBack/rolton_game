...
$(board).on('add', function(e, info) {
	...
	img.src =candy.toString() + "-candy.png";
...
$(document).on("click touchstart", "#canvas", function(evt){
	...
	if ($("img").is(':animated') == false){
		...
		if (img != null){
			$(img).css("z-index", 2);
			if(currentChosed)
				currentChosed.style.border=null;
			currentChosed=img;
	...
	if (dragDropInfo != null){
		...
		if (dragDropInfo.initCol == col && dragDropInfo.initRow-1 == row){
			if (rules.isMoveTypeValid(candy, "up")){
				...
				candy2 = board.getCandyInDirection(candy, "up");
			}
		}
		//down
			...
			if (rules.isMoveTypeValid(candy, "down")){
				...
				candy2 = board.getCandyInDirection(candy, "down");
			}
		}
		//left
			...
			if (rules.isMoveTypeValid(candy, "left")){
				...
				candy2 = board.getCandyInDirection(candy, "left");
		...
		//right
			...
			if (rules.isMoveTypeValid(candy, "right")){
				...
				candy2 = board.getCandyInDirection(candy, "right");
			}
		}
		currentChosed.style.border=null;
	    if(candy2) {
			...
			$(board).triggerHandler("move", details1);
			$(board).triggerHandler("move", details2);
			$(board).triggerHandler("move", details3);
			$(board).triggerHandler("move", details4);	
		}
